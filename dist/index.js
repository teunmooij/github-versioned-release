...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.run = void 0;
const create_commit_1 = __nccwpck_require__(9233);
const create_ref_1 = __nccwpck_require__(5362);
const get_tags_1 = __nccwpck_require__(9686);
const run = (args, octokit) => __awaiter(void 0, void 0, void 0, function* () {
    const tags = yield (0, get_tags_1.getTags)(args);
    const sha = yield (0, create_commit_1.createCommit)(args, octokit);
    for (const tag of tags) {
        yield (0, create_ref_1.createRef)(tag, sha, octokit);
    }
    return sha;
});
exports.run = run;
...
const glob_patterns_1 = __nccwpck_require__(2502);
const createCommit = ({ template, include, exclude, version }, octokit) => __awaiter(void 0, void 0, void 0, function* () {
    // Determine which files to include
    const files = yield (0, glob_1.glob)((0, glob_patterns_1.getIncludePatterns)({ template, include }), {
        ignore: yield (0, glob_patterns_1.getExcludePatterns)(exclude),
        nodir: true,
        cwd: process.env.GITHUB_WORKSPACE,
    });
    // Create the commit
    const [owner, repo] = process.env.GITHUB_REPOSITORY.split('/');
    const treePromises = files.map((path) => __awaiter(void 0, void 0, void 0, function* () {
        return ({
            path,
            mode: '100644',
            type: 'blob',
            content: yield node_fs_1.default.promises.readFile((0, node_path_1.join)(process.env.GITHUB_WORKSPACE, path), 'utf8'),
        });
    }));
    const tree = yield octokit.git.createTree({
        owner,
        repo,
        tree: yield Promise.all(treePromises),
    });
...
const createRef = (tag, sha, octokit) => __awaiter(void 0, void 0, void 0, function* () {
    const [owner, repo] = process.env.GITHUB_REPOSITORY.split('/');
    const ref = `tags/v${tag}`;
    const { data: matchingRefs } = yield octokit.git.listMatchingRefs({
        owner,
        repo,
        ref: ref,
    });
...
const getEventData = () => __awaiter(void 0, void 0, void 0, function* () {
    const eventPath = process.env.GITHUB_EVENT_PATH;
    if (!eventPath)
        return {};
    return Promise.resolve(`${eventPath}`).then(s => __importStar(require(s)));
});
...
const get_event_data_1 = __nccwpck_require__(92);
const semver_1 = __importDefault(__nccwpck_require__(1383));
const getTags = ({ version }) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    const eventData = yield (0, get_event_data_1.getEventData)();
    const versionTag = semver_1.default.valid(version || ((_a = eventData.release) === null || _a === void 0 ? void 0 : _a.tag_name));
    if (!versionTag)
        throw new Error('Invalid version');
    const tags = [versionTag];
    if (process.env.GITHUB_EVENT_NAME === 'release') {
        if (!((_b = eventData.release) === null || _b === void 0 ? void 0 : _b.draft) && !((_c = eventData.release) === null || _c === void 0 ? void 0 : _c.prerelease)) {
            const major = semver_1.default.major(versionTag).toString();
            const minor = semver_1.default.minor(versionTag).toString();
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getExcludePatterns = exports.getIncludePatterns = void 0;
const node_fs_1 = __importDefault(__nccwpck_require__(7561));
...
const extractNames = (input) => input
    .split('\n')
    .map(name => {
    ...
    .filter(name => name);
const getIncludePatterns = ({ template, include }) => {
    const list = [];
    if (template) {
...
const getExcludePatterns = (exclude) => __awaiter(void 0, void 0, void 0, function* () {
    if (exclude) {
        return extractNames(exclude);
    }
    if (!node_fs_1.default.existsSync('.gvrignore'))
        return [];
    const ignoreFile = yield node_fs_1.default.promises.readFile('.gvrignore', 'utf8');
...
(0, action_1.run)(args, octokit).then(sha => core.setOutput('sha', sha));
